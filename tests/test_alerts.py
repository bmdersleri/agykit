...
def test_threshold_detection(monkeypatch, tmp_path):
    ...
    result = alerts.check_quota_alerts(15, state_path=state, channels=["log"],
...
def test_cooldown_dedup(monkeypatch, tmp_path):
    ...
    first = alerts.check_quota_alerts(15, state_path=state, channels=["log"], ops_log_path=ops)
    assert len(first["alerts"]) == 1

    second = alerts.check_quota_alerts(15, state_path=state, channels=["log"], ops_log_path=ops)
...
def test_cooldown_expired(monkeypatch, tmp_path):
    ...
    result = alerts.check_quota_alerts(15, state_path=state, channels=["log"], ops_log_path=ops)
    assert len(result["alerts"]) == 1
...
def test_desktop_skip_when_absent(monkeypatch, tmp_path):
    ...
    result = alerts.check_quota_alerts(15, state_path=state, channels=["desktop"], ops_log_path=ops)
